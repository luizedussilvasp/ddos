...
def attack(ip, port, time, size):
    ...
    fmt = 'Atacando {ip} na porta {port} {time} com tamanho de {size} bytes.'.format(
        ip=ip,
        port='port {port}'.format(port=port) if port else 'QUALQUER',
        time='{time} seconds'.format(time=time) if str(time).isdigit() else 'SEM PAUSA',
        size=size
    )
    print(fmt)
